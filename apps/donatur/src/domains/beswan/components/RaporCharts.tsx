...
import type { ChartBulanan, ChartIPK as ChartIPKData } from "../services";

// Mirror RaporCharts portal internal — warna seri ikut token chart tema.
...
export function ChartKehadiranNilai({ data }: { data: ChartBulanan[] }) {
  const legend = [
...
export function ChartIPK({ data }: { data: ChartIPKData[] }) {
  const rows = data.map((d) => ({
