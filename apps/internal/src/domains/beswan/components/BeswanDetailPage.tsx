...
function RaporSummary({ rapor }: { rapor: BeswanRapor }) {
  ...
  return (
    <div className="grid md:grid-cols-3 gap-4">
      {rows.map((r) => (
        <div key={r.label} className="rounded-xl border bg-card p-4 shadow-sm space-y-2">
  ...
  return (
    <div className="flex min-h-[30vh] items-center justify-center rounded-xl border border-dashed">
      <div className="text-center text-sm text-muted-foreground px-4">
        <Construction className="mx-auto mb-2 h-6 w-6 opacity-50" />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <h1 className="text-xl font-bold">{detail.nama_lengkap}</h1>
      </div>
      ...
      {tab === "Absensi" && <AbsensiTab rapor={detail.rapor} />}
      {tab === "Tugas" && <TugasTab />}
      {tab === "Refleksi" && <RefleksiTab beswanId={beswanId} active={tab === "Refleksi"} />}
