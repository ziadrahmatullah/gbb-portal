...
import { ArrowLeft, Pencil, Trash2, Loader2, Tag, Clock } from "lucide-react";
  ...
  if (!cat) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3 text-muted-foreground">
        ...
        <Button variant="outline" onClick={() => navigate("/panel/master/categories")}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => navigate("/panel/master/categories")}>
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <div>
            <h1 className="text-xl font-bold leading-tight">{cat.name}</h1>
            <p className="text-xs text-muted-foreground mt-0.5">Category · {formatDate(cat.created_at)}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <Button variant="outline" size="sm" onClick={() => navigate(`/panel/master/categories/${id}/edit`)}>
      ...
      <div className="rounded-xl bg-card shadow-md px-5 py-4 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground capitalize">{cat.category_type}</p>
        </div>
      </div>
      ...
      <Card className="max-w-xl">
        ...
        <CardContent className="pt-0">
          <InfoRow icon={Tag} label="Name" value={cat.name} />
          <InfoRow icon={Clock} label="Created On" value={formatDate(cat.created_at)} />
        </CardContent>
      </Card>
