...
import { Plus, Eye, Pencil, Trash2, Loader2, MapPin, Calendar, Filter, Search, X, ArrowLeft, ArrowRight } from "lucide-react";
import { useMosques, useDeleteMosque } from "../hooks";
...
export function MosqueList() {
  ...
  const params = {
    ...
    limit,
    ...(search && { search }),
    ...(districtFilter !== "all" && { district_id: districtFilter }),
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage mosque data for qurban management.
            {!isLoading && <span className="font-medium ml-1">{pagination.total_item || 0} total</span>}
          </p>
        </div>
        <Button onClick={() => navigate("/panel/master/mosques/new")} className="flex items-center gap-2">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground gap-3">
          ...
          <p className="text-sm">
            {search || districtFilter !== "all"
              ? "No mosques found matching your criteria."
              : "No mosques yet. Add your first one."}
          </p>
          <Button variant="outline" onClick={() => navigate("/panel/master/mosques/new")}>
        ...
        <div className="rounded-xl border overflow-hidden">
          {items.map((item, idx) => (
            <div
              ...
              onClick={() => navigate(`/panel/master/mosques/${item.id}`)}
            >
              {/* Icon */}
              ...
              <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                ...
                <Button
                  ...
                  size="icon"
                  className="h-8 w-8"
                  onClick={() => navigate(`/panel/master/mosques/${item.id}/edit`)}
                  title="Edit"
                >
      ...
      {/* Pagination */}
      {!isLoading && pagination.total_item > 0 && (
