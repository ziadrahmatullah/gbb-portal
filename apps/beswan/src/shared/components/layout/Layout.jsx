...
import { Home, BookOpen, GraduationCap, NotebookPen, User, LogOut, Moon, Sun, Menu } from "lucide-react";
...
const navItems = [
  ...
  { to: "/refleksi", icon: NotebookPen, label: "Refleksi" },
        ...
        <div className="border-t p-3">
          <div className="flex items-center gap-3 px-2 py-1">
            <div className="rounded-full bg-accent flex items-center justify-center h-9 w-9 text-xs font-bold uppercase shrink-0">
              {user?.nama_lengkap ? user.nama_lengkap.slice(0, 2) : "BS"}
            </div>
            <div className="flex flex-col min-w-0">
              <span className="font-semibold text-sm truncate">{user?.nama_lengkap || "Beswan"}</span>
